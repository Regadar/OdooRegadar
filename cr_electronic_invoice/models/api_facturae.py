...
def gen_xml_v43(inv, sale_conditions, total_servicio_gravado,
                ...
                otro_texto_tag=False, otro_contenido_tag=False):
...
def consulta_documentos(inv, env, token_m_h, date_cr):
    ...
    if inv.move_type in ['out_invoice', 'out_refund']:
        ...
        if xml_firmado:
            inv.fname_xml_comprobante = inv.tipo_documento + clave + '.xml'

            # inv.xml_comprobante = xml_firmado
            inv.env['ir.attachment'].sudo().create({'name': inv.fname_xml_comprobante,
                                                     'type': 'binary',
                                                     'datas': xml_firmado,
                                                     'res_model': inv._name,
                                                     'res_id': inv.id,
        ...
        if xml_firmado:
            inv.fname_xml_comprobante = 'AHC_' + clave + '.xml'
    ...
    if (estado_m_h in ['aceptado', 'rechazado']) or (inv.move_type in ['out_invoice', 'out_refund']):
        inv.fname_xml_respuesta_tributacion = 'AHC_' + clave + '.xml'

        # inv.xml_respuesta_tributacion = response_json.get('respuesta-xml')
        response_rec = inv.env['ir.attachment'].sudo().create({'name': inv.fname_xml_respuesta_tributacion,
                                          'type': 'binary',
    ...
    if inv.tipo_documento != 'FEC' and estado_m_h == 'aceptado' and (not last_state or last_state == 'procesando'):
        ...
        if inv.move_type in ['in_invoice', 'in_refund']:
            email_template = inv.env.ref('cr_electronic_invoice.email_template_invoice_vendor', False)
        else:
            email_template = inv.env.ref('account.email_template_edi_invoice', False)
...
def load_xml_data(invoice, load_lines, account_id, product_id=False, analytic_account_id=False, interactive=False):
    try:
    ...
    invoice.economic_activity_id = invoice.company_id.activity_id
    invoice.partner_economic_activity_id = activity
    ...
    invoice.tipo_documento = fe_enums.TipoDocumentoFromXMLNode[document_type]
    ...
    if receptor != invoice.company_id.vat:
        if interactive:
            ...
            receiver_company_id = invoice.env['res.company'].search([('vat', '=', receptor)], limit=1)
    ...
    if not partner:
        tipo_emisor = invoice_xml.xpath("inv:Emisor/inv:Identificacion/inv:Tipo", namespaces=namespaces)[0].text ##TODO: check if this is correct or needs to search real db id
        identification_id = invoice.env['identification.type'].search([('code', '=', tipo_emisor)], limit=1).id
        ...
        state_id = invoice.env['res.country.state'].search([('code', '=', provincia)], limit=1).id
        canton = invoice_xml.xpath("inv:Emisor/inv:Ubicacion/inv:Canton", namespaces=namespaces)[0].text
        county_id = invoice.env['res.country.county'].search([('code', '=', canton),
        ...
        district_id = invoice.env['res.country.district'].search([('code', '=', distrito),
        ...
        if payment_method_node:
            payment_methods_id = invoice.env['payment.methods'].search([('sequence',
                                                                                '=',
                                                                                payment_method_node[0].text)], limit=1)
            info['payment_methods_id'] = payment_methods_id.id
        else:
            info['payment_methods_id'] = 1
        partner = invoice.env['res.partner'].create( info)
        if not partner:
            if interactive:
    ...
    if partner and partner.expense_account_id:
        default_account = partner.expense_account_id
        load_lines = partner.load_lines
    else:
        default_account = account_id

    if partner and partner.expense_analytic_account_id:
        default_analytic_account = partner.expense_analytic_account_id
    else:
        default_analytic_account = analytic_account_id

    invoice.partner_id = partner

    invoice.invoice_payment_term_id = partner.property_supplier_payment_term_id

    invoice.payment_methods_id = payment_methods_id or partner.payment_methods_id

    _logger.debug('FECR - load_lines: %s - account: %s', load_lines, default_account.name)
        ...
        for line in lines:
            ...
            for tax_node in tax_nodes:
                ...
                if product_id and product_id.non_tax_deductible:
                    tax = invoice.env['account.tax'].search([('company_id', '=', invoice.company_id.id),
                                                             ('tax_code', '=', tax_code),
                                                             ('amount', '=', tax_amount),
    ...
    elif not invoice.invoice_line_ids:
            columns = {'name': 'Total sin impuestos',
                       'move_id': invoice.id,
                       'price_unit': invoice.amount_total_electronic_invoice,
                       'quantity': 1,
                       #'product_uom_id': product_uom,
                       'sequence': 1,
                       #'discount': discount_percentage,
