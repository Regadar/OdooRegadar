...
class AccountInvoiceElectronic(models.Model):
    ...
    partner_economic_activity_id = fields.Many2one(
        comodel_name="economic.activity",
        string="Partner Economic Activity",
        context={
            'active_test': False
        }
    )
    partner_economic_activities_ids = fields.Many2many(
        comodel_name='economic.activity',
        string='Economic activities',
        compute='_compute_partner_economic_activities',
        context={
            'active_test': False
        }
    )
    ...
    def _compute_partner_economic_activities(self):
        for inv in self:
            if inv.move_type in ('in_invoice', 'in_refund'):
                if inv.partner_id:
                    inv.partner_economic_activities_ids = inv.partner_id.economic_activities_ids if inv.partner_id.economic_activities_ids else False
                    if not inv.partner_economic_activity_id:
                        inv.partner_economic_activity_id = inv.partner_id.activity_id
                else:
                    inv.partner_economic_activities_ids = self.env['economic.activity'].search([('active', '=', False)])
                    if not inv.partner_economic_activity_id:
                        inv.partner_economic_activity_id = inv.company_id.activity_id.id
            else:
                inv.partner_economic_activity_id = False
                inv.partner_economic_activities_ids = False
            #    inv.economic_activities_ids = self.env['economic.activity'].search([('active', '=', False)])
            #    if not inv.economic_activity_id:
            #        inv.economic_activity_id = inv.company_id.activity_id.id
    ...
    def load_xml_data(self):
        ...
        api_facturae.load_xml_data(self, load_lines, default_account, default_product, default_analytic_account, interactive=True)
    ...
    def _check_hacienda_for_invoices(self, max_invoices=10):
        ...
        for i in invoices:
            try:
                ...
                token_m_h = api_facturae.get_token_hacienda( i.company_id )
                        ...
                        if not inv.xml_comprobante or inv.state_invoice_partner not in ['procesando', 'aceptado']:
                            ...
                            if inv.state_tributacion != 'procesando':
                                ...
                                if 200 <= status <= 299:
                                    ...
                                    inv.state_tributacion = 'error'
                                _logger.error(
                                    'E-INV CR - Invoice: %s  %s sending Acceptance Message: %s',
                                    inv.number_electronic, inv.state_tributacion,
    ...
    def action_check_hacienda(self):
        for inv in self:
            if inv.company_id.frm_ws_ambiente != 'disabled':
                token_m_h = api_facturae.get_token_hacienda( inv.company_id )
                api_facturae.consulta_documentos(inv, self.company_id.frm_ws_ambiente, token_m_h, False)
        ...
        for inv in invoices:
            try:
                ...
                if days_left < 0:
                    ...
                    inv.state_tributacion = 'error'
                    continue
                elif days_left <= inv.company_id.range_days:
                    message = inv.company_id.get_message_to_send()
                    inv.message_post(
                        body=message,
                        subject=_('IMPORTANT NOTICE!!'),
                ...
                if not inv.xml_comprobante or (inv.tipo_documento == 'FEC' and inv.state_tributacion == 'rechazado'):
                    ...
                    currency_rate = round(currency._get_conversion_rate(currency, inv.company_id.currency_id, inv.company_id, date_cr), 5)
                    ...
                    for inv_line in inv.invoice_line_ids.filtered(lambda x: x.display_type == 'product'):
                        ...
                        if inv_line.product_id and inv_line.product_id.id == env_iva_devuelto:
                            ...
                            if not inv_line.product_uom_id or inv_line.product_uom_id.category_id.name in categories or inv_line.product_id.detailed_type == 'service':
                                if taxes:
                                    if _tax_exoneration:
                    ...
                    (otro_texto_tag, otro_contenido_tag) = api_facturae.gen_other_tags(inv)
    ...
    def create(self, vals_list):
        for vals in vals_list:
            ...
            type_override = vals.get('type_override')
            if type_override:
                vals['move_type'] = type_override
                vals.pop('type_override')
    ...
    def _post(self, soft=True):
        ...
        for inv in self:
            _logger.error('E-INV CR - estado FE: %s', inv.state_tributacion)
            if inv.company_id.frm_ws_ambiente == 'disabled':
                ...
                inv.partner_id.vat and inv.xml_supplier_approval is False: # FEC
                pass
            elif inv.move_type not in ('out_invoice', 'out_refund'):
                super(AccountInvoiceElectronic, inv)._post(soft)
                continue
            elif inv.tipo_documento == 'disabled':
                super(AccountInvoiceElectronic, inv)._post(soft)
                continue
            elif inv.state_tributacion == 'aceptado' or inv.state_tributacion == 'na':
                _logger.error('E-INV CR - ###### 1')
                super(AccountInvoiceElectronic, inv)._post(soft)
                inv.name = inv.number_electronic
                continue
            ...
            if not inv.company_id.signature or not inv.company_id.frm_ws_identificador or not inv.company_id.frm_ws_password or not inv.company_id.frm_pin:
                raise UserError(_('Please complete the electronic invoice information in the company profile'))
            if not inv.payment_methods_id or not inv.payment_methods_id.sequence:
                raise UserError(_('Please complete the payment method'))
